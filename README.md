...
## It runs with no setup at all

You do not need a `.env` file. Everything has a default, so you can clone this and have
it running in a minute. Here is what you get in that mode, so nothing surprises you.
...
long post will probably fail LinkedIn's eight sentence limit. Short posts come out
clean on all three platforms.
...
Add the two keys below and you get the real thing.
...
|---|---|---|
| `GEMINI_API_KEY` | Turns on AI generation. Free tier, no card, from [Google AI Studio](https://aistudio.google.com). Leave it empty and you get templates. | empty |
| `GEMINI_MODEL` | Which model to call. | `gemini-3.6-flash` |
| `DISCORD_WEBHOOK_URL` | Turns on real Discord publishing. Channel settings, Integrations, Webhooks. | empty |
...
**Schedule one.** `POST /variants/{id}/schedule` with a UTC time in the future. Try it
on a draft first and you will get a 409.
...
**A crash does not duplicate.** If the process dies mid-publish the attempt is left as
in progress. On restart those get marked failed rather than retried. I cannot tell
whether the message actually went out, and if I retry and it did, there are now two
posts. Marking it failed might leave a message that shows as failed but sent, which is
at least visible in the history for someone to check. One of those is fixable and the
other is not.
...
variants are cut on character count and can end mid-sentence, and long posts may fail
the LinkedIn sentence limit. That is the fallback behaving as designed.
...
**Some sites refuse to be fetched.** Wikipedia sends a 403 to anything that does not
look like a browser. The fetcher says who it is rather than pretending, so those fail
with a clear message and you paste the content instead.

**Title detection is rough.** A page with no usable title tag falls back to the first
line of text.

**Publishing can be up to 30 seconds late**, because the worker polls instead of firing
on an exact trigger. That is on purpose. The slots table is the source of truth, so a
slot created while the worker was down still gets picked up.
